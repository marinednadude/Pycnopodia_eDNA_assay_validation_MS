---
title: 'Lesperance eLowQuant LoD and LoQ: Stanford ddPCR 2 droplet minimum'
author: "Zachary Gold"
date: "2026-04-09"
...
DAT_RAW <- read.csv(FILE.PATH) %>% rename(SQ=TargetConcentration, Concentration=MeasuredConcentration) %>% 
  ...
  mutate(PositiveDroplets = if_else(PositiveDroplets < 2,0,PositiveDroplets)) %>% 
...
#  nn0 <- 24   
  ...
  knitr::kable(cbind(NumDetects=0:(nn0-1), SQS0.fit[,5:6]), 
...
for(i in 1:nTargets){
  ...
  betas.lower <- pmax(0, betas - 1.96 * (Calib.fit.estimates[[i]])[,2]) +0.01
...
Calib.choice <- Calib.fit.LLRp
xdxq.choice <- cbind(InterModel=Calib.choice, alphabeta.se, xdxq.all)
...
### Intercept model Results: Estimates, Critical, Detection, Quantification Limits and CIs
...
knitr::kable(round(xdxq.all.PR, 
                    digits=1), 
                    format="pandoc", digits=1, 
                     results="asis")

write.csv(xdxq.all.PR, file=here("LOD_LOQ","eLowQuant_Stanford_ddPCR_2_droplet.csv"), row.names = F)
```
...
```
**Convergence of 0 demonstrates successful model run**
...
#      ylim=c(0, maxlamhat), xlim=c(0, maxSQ), las=1, 
       main=paste('Intercept',uTargets[Targeti]))
  abline(Calib.fit$par[1], Calib.fit$par[2], col=4)
  legend("topleft", legend=c('ML fit'), lty=1, col=4, bty="n")
  arrows(Target.dat$SQ, Target.dat$Lamhatex.Lower, Target.dat$SQ, 
         Target.dat$Lamhatex.Upper,
...
max.xd_upper <- max(xd_upper.all[ManuTargets,])
...
max.xq_lower <- max(xq_lower.all[ManuTargets,])
