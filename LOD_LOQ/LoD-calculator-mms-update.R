...
DAT <- read.csv("Standard Curve Files/pycno-fullSC-data-Hakai.csv")
...
##If ddPCR, define template & rxn volumes
RXN.VOLUME <- 20
TEMPLATE.VOLUME <- 2
...
## For ddPCR, expect: Target, Concentration, SQ (or Expected_Conc)
...
if(ASSAY.TYPE == "ddPCR") {
  if(sum(colnames(DAT)=="Concentration")!=1) {
    A <- grep("concentration|conc|copies",colnames(DAT),ignore.case=TRUE)
    if(length(A)==1) { colnames(DAT)[A] <- "Concentration" }
    if(length(A)!=1) { write("There is a problem with the 'Concentration' column.\n\n",file="Analysis Log.txt",append=TRUE) }
    if(length(A)>1) { cat("ERROR: multiple 'Concentration' columns detected.",colnames(DAT)[A],sep="\n") }
    if(length(A)==0) { print("ERROR: cannot detect 'Concentration' column.") }
  }
  DAT$Concentration <- suppressWarnings(as.numeric(as.character(DAT$Concentration)))
}
...
if(ASSAY.TYPE == "ddPCR") {
  DAT$Concentration <- suppressWarnings(as.numeric(as.character(DAT$Concentration))) #Non-numerical values will be converted to NAs
  ##CHECK THAT THESE CALCULATIONS ARE RIGHT
  DAT$Concentration <- DAT$Concentration*RXN.VOLUME
  DAT$SQ <- DAT$SQ*TEMPLATE.VOLUME
}
...
if(ASSAY.TYPE == "ddPCR") {
  ...
  curve.list <- ""  # No curves needed for ddPCR
}
