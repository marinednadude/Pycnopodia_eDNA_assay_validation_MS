...
```{r detect, echo=FALSE, warnings=FALSE}
...
knitr::kable(DAT.Tar.SQ[DAT.Tar.SQ$Lab, c(1:4, 12)], digits=3, caption=paste(uLabsTargets$Lab,uLabsTargets$Target), format = "markdown")
...
## Compute fitted values for ML model
  ...
  plot(Target.dat$SQ, Target.dat$lamhat, xlog=TRUE, 
       ...
       main=paste0("No Intercept Model: Calibration Curve on Lambda Scale ", uTargets[i]))
...
##  sqs <- seq(0, maxSQ)
  ...
  plot(sqs, Calib.phat, xlog=TRUE, 
       ...
       ylim=c(0, 1), xlim=c(0, maxSQa),  main=paste0("No Intercept Model: Calibration Curve on Phat Scale ", uTargets[i]))
...
cat('\n\n','ML estimate of SQ for nd0 number of detects and nn0 replicates', '\n')
  ...
  kable(CalibS0.fit.estimates0,  format="pandoc", digits=3)
```
...
i=1
  ...
  knitr::kable(cbind(NumDetects=0:(nn0-1), SQS0.fit0[,3:4]), 
                     format="pandoc", digits=3)


  CalibS0.table0[[i]] <- SQS0.fit0
  kable(CalibS0.table0, format="pandoc", digits=3)
...
#### Intercept model: Estimates, Critical, Detection, Quantification Limits and CIs
...
#### No intercept model: Estimates, Critical, Detection, Quantification Limits and CIs
...
knitr::kable(round(xdxq.all0.PR, 
                    digits=1), 
                    format="pandoc", digits=1, 
                     results="asis")
```
...
####  Estimates, Critical, Detection, Quantification Limits and CIs
...
knitr::kable(round(xdxq.choice.PR, 
                    digits=1), 
                    format="pandoc", digits=1, 
                     results="asis")
```
...
#  } else {
  ...
  plot(Target.dat$SQ, Target.dat$lamhat, xlog=TRUE, 
       ...
       ylim=c(0, 4), xlim=c(0, maxSQ), las=1, 
 #     ylim=c(0, maxlamhat), xlim=c(0, maxSQ), las=1, 
      main=paste('No intercept', uTargets[Targeti]))
  abline(0, Calib.fit$par, col=4)
  legend("topleft", legend=c('ML fit'), lty=1, col=4, bty="n")
  arrows(Target.dat$SQ, Target.dat$Lamhatex.Lower, Target.dat$SQ, 
         Target.dat$Lamhatex.Upper,
