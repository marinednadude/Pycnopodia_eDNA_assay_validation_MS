...
pycno_standard_threshold %>% 
  ...
  arrange(desc(Quantity)) %>% kable(digits=2)
...
pycno_results_standard %>% 
 ...
 mutate(., Observed_Quantity= if_else(detection=="Detection",10^((CT -`Y-Intercept`)/Slope), 0 )) %>%
  ...
  group_by(`Sample Name`,Quantity) %>% 
   dplyr::summarise(., mean_conc = round(mean(Concentration_L),2), sd_conc = round(sd(Concentration_L),2), n()) %>% mutate(., perc_sd_conc = round(sd_conc/mean_conc*100,2))%>%  arrange(Quantity) %>%  kable(digits=2)
...
Before getting started, please read the notes and personalize lines 17, 20, 23, 26,and 33.  There are interactive codes, it is designed to be run using R studio 
...
```{r}
write(paste0("Analysis started: ",date(),"\n\n"),file=here("Analysis Log.txt"))
```

```{r}

DAT %>% 
  dplyr::select(Target=`Target Name`, Cq=CT, Sq=Quantity) -> DAT
```
...
## Identify any wells where the Cq value is more than 10% away from the median for that standard.
```{r}
...
for(i in 1:length(Targets)) {
  ...
  Decay.Plot <- ggplot(DAT2[DAT2$Target==Targets[i],], aes(x= Standards, y = Cq.CV)) +
    ...
    geom_vline(xintercept=DAT3$LOQ[DAT3$Assay==Targets[i]],color="blue") +
    annotate("text",y=max(DAT2$Cq.CV[DAT2$Target==Targets[i]],na.rm=TRUE)*0.99,
             x=DAT3$LOQ[i]*0.8,angle=90,label="LOQ",color="blue") +
...
## Create an analysis log file:
write(paste0("Analysis started: ",date(),"\n\n"),file=here("eLowQuant_output","Outputs\\Analysis Log.txt"))
...
{
if(sink.indicator){
  sink(file=here("eLowQuant_output",'Outputs\\Summaries.txt'), split=TRUE)}
for(i in uLabs){
...
## If desired, set sink.indicator to TRUE to write results to file.  
  if(sink.indicator){
    sink(file(here("eLowQuant_output",paste("Outputs\\MLNoInter",uTargets[i],".txt", sep="")), 
...
#  nd0 <- 1; nn0 <- 3   
  ...
  if(sink.indicator) sink(file(here("eLowQuant_output",paste("Outputs\\MLNoInterSQ",uTargets[i],
            ".txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
...
#  nn0 <- 24   
  ...
  if(sink.indicator) sink(file(here("eLowQuant_output",paste("Outputs\\MLNoInterSQ0",uTargets[i],
            ".txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
...
## If desired, write results to file.
  if(sink.indicator){
    sink(file(here("eLowQuant_output",paste("Outputs\\ML",uTargets[i],".txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
  }
  cat("\n", as.character(uTargets[i]), "\n")
  cat("Convergence=", Calib.fit$convergence, "\n")
...
```{r MLES0fits, warning=FALSE, echo=FALSE, eval=FALSE, include=FALSE}  
  ...
  cat('\n\n','ML estimate of SQ for nd0 number of detects and nn0 replicates', '\n')
  if(sink.indicator) sink(file(here("eLowQuant_output",paste("Outputs\\MLSQ",uTargets[i],".txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
  cat(as.character(uTargets[i]), 'y0=', nd0, 'n0=' , nn0, "\n\n")
  printCoefmat(cmat, digits=3)
  if(sink.indicator) sink()
...
#  nn0 <- 24   
  ...
  cat(' ML estimate of SQ for numbers of detects and', nn0, 'replicates', '\n')
  if(sink.indicator) {
    sink(file(here("eLowQuant_output",paste("Outputs\\MLSQ0",uTargets[i],".txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
  }
...
{
if(Manusink){ 
      sink(file(here("eLowQuant_output",paste("Outputs\\LimitsAll.txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
}
...
{
if(Manusink){ 
      sink(file(here("eLowQuant_output",paste("Outputs\\LimitsAll0.txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
}
...
{
if(Manusink){ 
      sink(file(here("eLowQuant_output",paste("Outputs\\Limits.txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
}
...
{
if(Manusink){
  sink(file(here("eLowQuant_output","Outputs\\Data.txt"), encoding="UTF-8"),
       split=TRUE)
  }
...
{
 if(Manusink){
    sink(file(here("eLowQuant_output",paste("Outputs\\ResultsAll.txt", sep="")), 
            encoding="UTF-8"), split=TRUE)
  }
