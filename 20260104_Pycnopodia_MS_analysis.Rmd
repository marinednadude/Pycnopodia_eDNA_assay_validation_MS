...
write.csv(DAT, file = here("LOD_LOQ/qPCR_Pycno_5_v3_standards.csv"), row.names = F)
...
results_data_cleaned_run1 %>% 
  ...
  mutate(., TemplateVolume =2,
         ReactionVolume= 10) %>% 
  dplyr::select(Target=`Target Name`, Lab, Cq=CT, SQ=Quantity) -> DAT2

write.csv(DAT2, file = here("LOD_LOQ/qPCR_Pycno_1_standards.csv"), row.names = F)
```
...
combined_qPCR_data_run1 %>% 
  ...
  filter(., !str_detect(`Task`,"STANDARD")) %>% 
      mutate(., Threshold = if_else(Rn > h_run1, "Above","Below")) %>% 
  group_by(`Well Position`,`Sample Name`) %>% 
  filter(., rank(Threshold, ties.method="first")==1) %>% 
...
ggsave(filename = here("Figures","FHL_mesocosm_field_trial.png"),
       width=10,
       height = 4,
       dpi = 400,
      units = c("in"))
